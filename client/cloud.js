...
drawImgCloud(x, y, cloudArr, size, num, sat) {
    if(num == 0) drawFromJson.draw(cloudArr, x/2 - 50, y - 50, 0.7, undefined, undefined, sat);
    else drawFromJson.draw(cloudArr, x - 50, y - 50, size, undefined, undefined, sat);
}
...
draw(isRainOrSnow) {
    ...
    if (this.cloudAmount > 0) {
        ...
        for (let i = 0; i < this.cloudAmount; i++) {
            ...
            if(isRainOrSnow) {
                this.drawImgCloud(this.clouds[i].xpos, this.clouds[i].ypos,
                    cloudArr[(i + 1) % 4], this.clouds[i].cloudScale,
                    (i + 1) % 4, CON.VALUE.cloud_saturation);
            } else {
                this.drawImgCloud(this.clouds[i].xpos, this.clouds[i].ypos, cloudArr[(i + 1) % 4], this.clouds[i].cloudScale, (i + 1) % 4);
            }
